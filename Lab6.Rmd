...
str(ks_data)
summary(ks_data)
ggplot(ks_data, aes(x=name, y=goal))+geom_point()
filter(ks_data, goal > 100000000)
```
...
For cleaning, we would need to fix the date and time objects to the correct datatype. The notes on the last three variables (usd pledged, usd.pledged.real, usd goal.real) are ambiguous about what they are for exactly. Those may need to be converted by hand. ID should probably be converted to a factor as well.